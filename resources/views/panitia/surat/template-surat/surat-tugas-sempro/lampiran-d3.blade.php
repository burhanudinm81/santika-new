...
<head>
    ...
    <title>Lampiran Surat Tugas Seminar Proposal</title>
    <style>
        body {
            ...
            <div class="content">
                @if ($pageIndex === 0)
                    <div class="table-header">
                        ...
                        <h4><strong>No Surat: {{ $formData['nomor_surat'] }}</strong></h4>
                    </div>
                @endif
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:center; width: 20%;">Mahasiswa</th>
                    ...
                    <tbody>
                        @foreach ($chunk as $index => $item)
                            <tr>
                                ...
                                <td rowspan="2">{{ $item->proposal->judul ?? "-" }}</td>
                                <td rowspan="2" class="left-text">Ketua:<br>{{ $item->proposal->dosenPengujiSempro1->nama ?? "-" }}<br>Anggota:<br>{{ $item->proposal->dosenPengujiSempro2->nama ?? "-" }}</td>
                            </tr>
                            <tr>
                                <td>{{ $item->proposal->proposalMahasiswas[1]->mahasiswa->nama ?? "-" }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                        ...
                        <div class="signature-section">
                            <div class="signature-content">
                                <p style="margin: 5px 0;">
                                    @if(isset($formData['tanggal_tanda_tangan']) && !empty($formData['tanggal_tanda_tangan']))
                                        Malang, {{ $formData['tanggal_tanda_tangan'] }}
                                    @else
                                        Malang, {{ date('d F Y') }}
                                    @endif
                                </p>
                                ...
                                <div class="signature-name">
                                    <p style="margin: 5px 0;">
                                        <strong>
                                            @if(isset($formData['nama_penandatangan']) && !empty($formData['nama_penandatangan']))
                                                {{ $formData['nama_penandatangan'] }}
                                            @else
                                                [Nama Penandatangan]
                                            @endif
                                        </strong>
                                    </p>
                                    <p style="margin: 5px 0; margin-top: -6px;">
                                        @if(isset($formData['nip_penandatangan']) && !empty($formData['nip_penandatangan']))
                                            NIP. {{ $formData['nip_penandatangan'] }}
                                        @else
