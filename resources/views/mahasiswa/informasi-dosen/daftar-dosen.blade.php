                        ...
                        <div class="input-group">
                            <select name="bidang_minat" class="form-control" onchange="this.form.submit()">
                                ...
                                @foreach ($bidangMinatList as $bm)
                                    <option value="{{ $bm->id }}" {{ ($bidangMinatId == $bm->id) ? 'selected' : '' }}>
                                        {{ $bm->bidang_minat }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card card-solid border-0" style="background-color: transparent;">
                                            ...
                                            <div class="card-body py-2 text-center">
                                                @if (is_null($dsn->foto_profil))
                                                    <img src="{{ url('/images/blank-profile-64x64.png') }}" alt="user-avatar"
                                                        class="img-circle img-fluid mx-auto d-block mb-2"
                                                ...
                                                @else
                                                    <img src="{{ $dsn->foto_profil }}" alt="user-avatar"
                                                        ...
                                                        style="width: 64px; height: 64px;">
                                                @endif
                                                <h2 class="h6 mt-1 mb-1">
                                                    <b>{{ \Illuminate\Support\Str::limit($dsn->nama ?? 'Nama Dosen', 20) }}</b>
                                                </h2>
                                                <p class="text-muted text-sm mb-1"
                                                    style="line-height: 1.2; height: 2.4em; overflow: hidden;">
                                                    {{ $dsn->nidn }}
                                                </p>
                                            </div>
                                            <div class="card-footer text-center py-1">
                                                <a href="{{ route('mahasiswa.informasi-dosen.profil-dosen', ['id' => $dsn->id]) }}"
                        ...
                        <!-- /.card-body -->
                        <div class="card-footer d-flex justify-content-center border-0 mb-0">
