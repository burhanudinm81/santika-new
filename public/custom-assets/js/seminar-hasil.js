$(document).ready(function () {
    ...
    $('#buttonTampilNilaiSemhasAkhirByPeriode').click(function () {
        const periodeId = $('#periode_semhas_id').val();
        const tahapId = $('#tahap_semhas_id').val();
        const prodiPanitiaId = $('#prodi_panitia_semhas_id').val();
        ...
        // /panitia/ajax/list-rekap-nilai-sempro <- url
        $.ajax({
            url: '/panitia/ajax/list-rekap-nilai-semhas-akhir',
            method: 'GET',
            data: {
                periode_id: periodeId,
                tahap_id: tahapId,
            ...
            success: function (response) {
                        ...
                        if (prodiPanitiaId == 1) {
                            tbody += `
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        <a href="/panitia/seminar-hasil/detail-nilai/${item.id}?id=${item.proposal_mahasiswas[0].mahasiswa_id}" class="btn btn-primary btn-sm">View</a>
                                    </td>
                                </tr>
                            `;
                        } else if (prodiPanitiaId == 2) {
                ...
                $('#data-nilai-akhir').html(tbody);
            },
            error: function (xhr) {
                alert('Terjadi kesalahan saat memuat data rekap nilai semhas akhir.');
            }
        });
    });
